...
## Output Anchoring Strategy

- All checked outputs are appended to `logs/output_log.jsonl` with a hash, verdict, and timestamp.
- `src/anchor_outputs.py` batches new log lines into a Merkle tree and anchors only the Merkle root on-chain (Sepolia) to keep costs low and UX fast.
- Each anchor entry is added here when run; a specific output can be proven by showing the log line plus its Merkle proof against the anchored root.
